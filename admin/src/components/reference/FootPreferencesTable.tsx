...
import {
  createFootPreference,
  deleteFootPreference,
  fetchFootPreferences,
  updateFootPreference,
  type FootPreference,
} from "@/actions/foot-preferences";

export default function FootPreferencesTable({
  initialRows,
}: {
  initialRows: FootPreference[];
}) {
  const { data, isLoading } = useQuery({
    queryKey: ["foot-preferences"],
    queryFn: fetchFootPreferences,
  ...
  const [selected, setSelected] = useState<FootPreference | null>(null);

  const refresh = () =>
    queryClient.invalidateQueries({ queryKey: ["foot-preferences"] });

  const openEdit = (r: FootPreference) => {
    setSelected(r);
    setDraftCode(r.code);
  ...
  const openDelete = (r: FootPreference) => {
    setSelected(r);
  ...
  const handleCreate = async () => {
    ...
    const res = await createFootPreference({
      code: draftCode.trim(),
    ...
    toast.success("Foot preference created");
    setIsAddOpen(false);
    setDraftCode("");
    setDraftDesc("");
    refresh();
  ...
  const handleUpdate = async () => {
    ...
    setIsSubmitting(true);
    const res = await updateFootPreference(selected.id, {
      code: draftCode.trim(),
      description: draftDesc.trim() ? draftDesc.trim() : null,
    });
    setIsSubmitting(false);
    ...
    toast.success("Foot preference updated");
    setIsEditOpen(false);
    setSelected(null);
    refresh();
  };
  ...
  const handleDelete = async () => {
    if (!selected) return;
    setIsSubmitting(true);
    const res = await deleteFootPreference(selected.id);
    setIsSubmitting(false);
    if (res && typeof res === "object" && "error" in res) {
      toast.error(String(res.error));
      return;
    }
    toast.success("Foot preference deleted");
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="bg-white w-[30rem]">
          ...
          <div className="flex flex-col gap-3">
            <Input value={draftCode} onChange={(e) => setDraftCode(e.target.value)} placeholder="Code" />
            <Input value={draftDesc} onChange={(e) => setDraftDesc(e.target.value)} placeholder="Description (optional)" />
          </div>
